...
def build_weekly_sessions():
    ...
    # v2: each section is already bound to one requirement
    # (sections.requirement_id). One weekly session per section
    # per sessions_per_week, minus existing DRAFT allocations
    # (fresh mode: existing is empty, so every section is scheduled).

    for srow in sections_df.itertuples(index=False):
        ...
        req = session_requirements.get(int(requirement_id))
        if req is None:
            continue

        term_id = int(srow.term_id)

        if not solver_existing_allocations.empty:
            existing_count = len(
                solver_existing_allocations[
                    (
                        solver_existing_allocations["section_id"].astype(int)
                        == section_id
                    )
                    &
                    (
                        solver_existing_allocations["requirement_id"].astype(int)
                        == int(requirement_id)
                    )
                ]
            )
        ...
        remaining_count = max(sessions_per_week - existing_count, 0)
        ...
        for i in range(remaining_count):
            ...
            rows.append({
                ...
                "section_code": str(srow.code),

                "requirement_id": int(requirement_id),

                "course_id": int(srow.course_id),

                "kind": str(req.get("kind", srow.kind)),

                "instance_number": int(instance_number),

                "duration_minutes": int(req["duration_minutes"]),

                "required_room_kind": req["required_room_kind"]
            })
...
# =================================================================
# Reusable Solver Builders (single source of truth)
# solve_schedule() and the teaching notebook both call these.
# =================================================================




def create_decision_variables(candidates, weekly_sessions):

    """Create the CP-SAT model with one BoolVar per candidate plus one UNSCHEDULED var per session."""
    ...
    return model, x, unscheduled




def add_session_coverage_constraints(model, x, unscheduled, candidates, weekly_sessions):

    """Every session takes exactly one candidate OR unscheduled."""
...
def add_room_conflict_constraints(model, x, candidates):

    """At most one candidate per room per slot."""
...
def add_instructor_conflict_constraints(model, x, candidates):

    """At most one candidate per instructor per slot."""
...
def add_section_conflict_constraints(model, x, candidates):

    """At most one candidate per section per slot."""
...
def add_student_conflict_constraints(model, x, candidates):

    """Sections sharing an ACTIVE student cannot share a slot (group fallback only without enrollments)."""

    # Falls back to group assignments when a section has no enrollments.
    # =================================================

    student_candidate_map = (
        defaultdict(list)
    )
    ...
    for row in candidates.itertuples(
        ...
        if not section_students:
            section_students = {
                f"g_{g}"
                for g in groups_by_section.get(int(row.section_id), set())
            }
...
def build_candidate_score(candidates, x):

    """Candidate-level soft-score expression."""
...
def find_adjacent_slots():

    """Find directly adjacent slot pairs per weekday."""
...
def build_occupancy_variables(model, x, candidates):

    """Occupancy indicators per section/instructor and slot."""
    ...
    return section_occupancy, instructor_occupancy




def build_compactness_terms(model, candidates, section_occupancy, instructor_occupancy, adjacent_slots):

    """Reward adjacent sessions for the same section or instructor."""
    ...
    return compactness_vars, compactness_bonus




def build_room_utilization_terms(model, x, candidates):

    """Prefer solutions using fewer active rooms."""
    ...
    return room_used, room_utilization_penalty




def build_unscheduled_cost(unscheduled):

    """Penalty expression for unscheduled sessions."""
...
def set_final_objective(model, candidate_score, compactness_bonus, room_utilization_penalty, unscheduled_cost):

    """Maximize score + compactness - room penalty - unscheduled cost."""
...
def solve_model(model, max_time_seconds=30, num_workers=2):
    ...
    return status, status_name, solver




def extract_solution(solver, x, unscheduled, candidates, weekly_sessions, compactness_vars, room_used):

    """Read the solved model: schedule, metrics and objective values."""
    ...
    return {
        ...
        "candidate_score_value": candidate_score_value,
        "selected_compact_pairs": selected_compact_pairs,
        "compactness_bonus_value": compactness_bonus_value,
        "active_room_ids": active_room_ids,
        "active_room_count": active_room_count,
        "room_penalty_value": room_penalty_value,
        "unscheduled_penalty_value": unscheduled_penalty_value,
        "calculated_objective": calculated_objective,
...
def get_ranked_alternatives(selected_schedule, candidates, session_key, limit=5):

    """Top conflict-free alternative candidates for one session."""
    ...
    def candidate_conflicts_with_selected(
        ...
        for selected in selected_rows:
            ...
            # Shared individual student (authoritative)
            selected_students = (
                students_by_section_ids.get(
                    int(selected.section_id),
                    set(),
...
def build_scheduled_payload(selected_schedule, candidates):

    """API payload for scheduled sessions with ranked alternatives."""
...
def build_unscheduled_report(weekly_sessions, diagnostics, unscheduled_sessions):

    """API payload for unscheduled sessions with diagnostic reasons."""
...
def solve_schedule(
    ...
    # ---------------------------------------------
    # 0) Term Readiness Gate (no CP-SAT on bad data)
    # ---------------------------------------------
    ...
    # =================================================
    # 8) Student Conflicts (authoritative enrollments)
    # Two sections sharing any ACTIVE student cannot share a slot.
    ...
    section_occupancy, instructor_occupancy = (
        build_occupancy_variables(model, x, candidates)
    )
    ...
    room_used, room_utilization_penalty = (
        build_room_utilization_terms(model, x, candidates)
    )
    ...
    status, status_name, solver = solve_model(
        model, max_time_seconds, num_workers
    )
    ...
    selected_schedule = details[
        "selected_schedule"
    ]


    unscheduled_sessions = details[
        "unscheduled_sessions"
    ]


    candidate_score_value = details[
        "candidate_score_value"
    ]


    selected_compact_pairs = details[
        "selected_compact_pairs"
    ]


    compactness_bonus_value = details[
        "compactness_bonus_value"
    ]


    active_room_ids = details[
        "active_room_ids"
    ]


    active_room_count = details[
        "active_room_count"
    ]


    room_penalty_value = details[
        "room_penalty_value"
    ]


    unscheduled_penalty_value = details[
        "unscheduled_penalty_value"
    ]


    calculated_objective = details[
        "calculated_objective"
    ]


    solver_objective = details[
        "solver_objective"
    ]
    ...
    scheduled_sessions = build_scheduled_payload(
    ...
    unscheduled_payload = build_unscheduled_report(
        weekly_sessions, diagnostics, unscheduled_sessions
    )
