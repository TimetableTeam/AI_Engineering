...
def validate_term_readiness(term_id, tables_override=None):
    ...
    # Unknown term: no requirements, sections or registrations at all.
    if (
        term_reqs.empty
        and sections_df[
            sections_df["term_id"].astype(int) == term_id
        ].empty
        and registrations_df[
            registrations_df["term_id"].astype(int) == term_id
        ].empty
    ):
        return {
            "ready": False,
            "errors": [{"code": "TERM_NOT_FOUND", "term_id": term_id}],
        }
